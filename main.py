...
bot = telebot.TeleBot("5570569628:AAGgdIsswRJ0KED4cnoBsSxIJLm48XBpDbg")
...
def addTest(message):
    ...
    @bot.message_handler(commands=['razdel4'])
    def add_theme(message4):
        listForTask.append(message4.text.split())
        print(message4.text.split())
        bot.send_message(message4.chat.id, "Введите тему ")

    @bot.message_handler(commands=["push"])
    def acept_changes(message5):
        try:
            try:
                mutex.acquire()
                print(listForTask)
                update_list_for_task = [listForTask[0][0], listForTask[1][0], listForTask[2][0], listForTask[3][0]]
